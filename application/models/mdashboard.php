...
{
	function get_info_statistik(){
		$data 		= array();
		$th = array();
		$data['jml_1'] = 0;
		$data['jml_2'] = 0;
	
		$temp_front = $this->db->query("SELECT COUNT(no_tiket_frontdesk) AS jml, YEAR(tanggal) AS thn FROM tb_tiket_frontdesk WHERE tanggal != '' GROUP BY YEAR(tanggal)")->result();
		
		$data['front'] = array();
		foreach($temp_front as $row){
			$data['front'][$row->thn] = $row->jml;
			$data['jml_1'] += $row->jml;
			$th[] = $row->thn;
		}
		
		$temp_help = $this->db->query("SELECT COUNT(no_tiket_helpdesk) AS jml, YEAR(tanggal) AS thn FROM tb_tiket_helpdesk 
WHERE tanggal != '' GROUP BY YEAR(tanggal)")->result();
		
		$data['help'] = array();
		foreach($temp_help as $row1){
			$data['help'][$row1->thn] = $row1->jml;
			$data['jml_2'] += $row1->jml;
			$th[] = $row1->thn;
		}
		
		$data['tahun'] = array_unique($th);
